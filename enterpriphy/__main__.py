"""enterpriphy CLI — `enterpriphy install` sets up the Claude Code skill.

The `graphify` CLI entry-point remains as a backward-compatible alias and
will be removed in 2.0. All user-facing skill paths and slash commands
(`/graphify`) are preserved during the migration window; Phase 0.5 of the
migration plan will add `/enterpriphy` as the primary command and keep
`/graphify` as an alias for one minor cycle.
"""
...
try:
    ...
    # Prefer the new package; fall back to the legacy graphifyy distribution
    # so wheels that have not yet been rebuilt still report a sane version.
    try:
        __version__ = _pkg_version("enterpriphy")
    except Exception:
        __version__ = _pkg_version("graphifyy")
except Exception:
    __version__ = "unknown"

# Output directory — override with ENTERPRIPHY_OUT (preferred) or the legacy
# GRAPHIFY_OUT env var for worktrees or shared-output setups.
_GRAPHIFY_OUT = os.environ.get("ENTERPRIPHY_OUT") or os.environ.get("GRAPHIFY_OUT", "graphify-out")
...
def _enforce_graph_size_cap_or_exit(gp: Path) -> None:
    ...
    """
    from enterpriphy.security import check_graph_file_size_cap
    try:
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    ...
    # Git hook
    try:
        from enterpriphy.hooks import uninstall as hook_uninstall
...
def main() -> None:
    ...
    if cmd == "install":
        ...
        from enterpriphy.prs import cmd_prs
        cmd_prs(sys.argv[2:])
    elif cmd == "hook":
        from enterpriphy.hooks import install as hook_install, uninstall as hook_uninstall, status as hook_status
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
        ...
        from enterpriphy.security import sanitize_label
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from enterpriphy.affected import DEFAULT_AFFECTED_RELATIONS, format_affected, load_graph
        query = sys.argv[2]
        ...
        from enterpriphy.ingest import save_query_result as _sqr
        out = _sqr(
        ...
        if len(sys.argv) < 4:
            ...
            sys.exit(1)
        from enterpriphy.serve import _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        segments = []
        from enterpriphy.build import edge_data
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from enterpriphy.serve import _find_node
        from networkx.readwrite import json_graph
        label = sys.argv[2]
        graph_path = _default_graph_path()
        args = sys.argv[3:]
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from enterpriphy.ingest import ingest as _ingest
        url = sys.argv[2]
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from enterpriphy.watch import watch as _watch
        try:
            _watch(watch_path)
        ...
        from enterpriphy.analyze import god_nodes, surprising_connections, suggest_questions
        from enterpriphy.report import generate
        from enterpriphy.export import to_json, to_html
        ...
        from enterpriphy.export import _git_head as _gh
        _commit = _gh()
        ...
        from enterpriphy.export import backup_if_protected as _backup
        _backup(out)
        ...
        from enterpriphy.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        ...
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from enterpriphy.build import prefix_graph_for_global as _prefix
        graphs = []
        ...
        if subcmd == "callflow-html":
            from enterpriphy.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from enterpriphy.build import build_from_json as _bfj
        ...
        if subcmd == "html":
            from enterpriphy.export import to_html as _to_html
            if no_viz:
            ...
            from enterpriphy.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            from enterpriphy.wiki import to_wiki as _to_wiki
            from enterpriphy.analyze import god_nodes as _god_nodes
            if not communities:
                print(
            ...
            if neo4j_uri:
                ...
                from enterpriphy.export import to_cypher as _to_cypher
                _to_cypher(G, str(out_dir / "cypher.txt"))
                print(f"cypher.txt written - import with: cypher-shell < {out_dir}/cypher.txt")

    elif cmd == "benchmark":
        from enterpriphy.benchmark import run_benchmark, print_benchmark
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from enterpriphy.global_graph import (
        ...
        from enterpriphy.detect import (
            detect as _detect,
        ...
        if code_files:
            from enterpriphy.extract import extract as _ast_extract
        ...
        if no_cluster:
            ...
            if global_merge:
                from enterpriphy.global_graph import global_add as _global_add
        ...
        from enterpriphy.build import (
            build as _build,
            build_from_json as _build_from_json,
            build_merge as _build_merge,
        )
        from enterpriphy.cluster import cluster as _cluster, score_all as _score_all
        from enterpriphy.export import to_json as _to_json
        from enterpriphy.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
        ...
        from enterpriphy.cache import check_semantic_cache
        if len(sys.argv) < 3:
