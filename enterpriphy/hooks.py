...
if [ -n "$GRAPHIFY_BIN" ]; then
    ...
    if [ -n "$GRAPHIFY_PYTHON" ] && ! "$GRAPHIFY_PYTHON" -c "import enterpriphy" 2>/dev/null; then
        GRAPHIFY_PYTHON=""
    fi
fi
# Fall back: try python3, then python (Windows has no python3 shim)
if [ -z "$GRAPHIFY_PYTHON" ]; then
    if command -v python3 >/dev/null 2>&1 && python3 -c "import enterpriphy" 2>/dev/null; then
        GRAPHIFY_PYTHON="python3"
    elif command -v python >/dev/null 2>&1 && python -c "import enterpriphy" 2>/dev/null; then
...
try:
    from enterpriphy.watch import _rebuild_code, _apply_resource_limits
